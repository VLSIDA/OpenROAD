...
void MoveTracker::trackCriticalPins(const vector<const Pin*>& critical_pins)
{
  ...
  // Don't use the provided list - instead iterate over all instances and output pins
  all_critical_pins_.clear();

  // Iterate through all instances in the design
  sta::LeafInstanceIterator* inst_iter = sta_->network()->leafInstanceIterator();
    ...
    for (const Pin* pin : critical_never_visited) {
      ...
      if (vertex) {
        ...
        while (in_edge_iter.hasNext()) {
          ...
          if (!from_pin
              || sta_->network()->direction(from_pin)->isOutput()) {
            continue;
          }
          ...
          for (const sta::RiseFall* rf : sta::RiseFall::range()) {
            ...
            const sta::Delay delay
                = sta_->graph()->arcDelay(prev_edge, prev_arc, dcalc_ap->index());
...
void MoveTracker::printSlackDistribution(const std::string& title)
{
  ...
  // Calculate integral scale factor: target ~50 characters but use integral scale
  constexpr int target_bar_width = 50;
  int gates_per_hash = (max_count + target_bar_width - 1) / target_bar_width;  // Ceiling division
  if (gates_per_hash == 0) {
    gates_per_hash = 1;
  }
  ...
  debugPrint(
      logger_, RSZ, "move_tracker", 1, "Pre-Optimization Slack Distribution:");
  debugPrint(logger_,
             RSZ,
             "move_tracker",
             1,
  ...
  debugPrint(logger_, RSZ, "move_tracker", 1, "");
  debugPrint(
      logger_, RSZ, "move_tracker", 1, "Post-Optimization Slack Distribution:");
  debugPrint(logger_,
             RSZ,
             "move_tracker",
             1,
  ...
  if (!initial_endpoint_slack_.empty()) {
    debugPrint(logger_, RSZ, "move_tracker", 1, "");
    debugPrint(logger_, RSZ, "move_tracker", 1, "=== Endpoint Slack Distribution ===");
    ...
    for (const auto& [endpoint_pin, initial_slack_ns] : initial_endpoint_slack_) {
      // Find which bin this slack falls into
      int bin = 0;
      for (size_t i = 0; i < bin_edges.size(); i++) {
        if (initial_slack_ns >= bin_edges[i]) {
    ...
    int endpoints_per_hash = (endpoint_max_count + target_bar_width - 1) / target_bar_width;  // Ceiling division
    ...
    // Display scale key with integral value
    string endpoint_scale_label = (endpoints_per_hash == 1) ? "endpoint" : "endpoints";
    debugPrint(logger_,
               RSZ,
               "move_tracker",
               1,
