...
| `-neighbor_check` | Enable the neighbor feasibility check: suppress a candidate move when an off-path neighbor it slows (a fanin driver whose load grows, the other pin of a commutative-pin swap, a rebuffered fanout, or a load moved behind a split buffer) becomes the governing worst slack of the move's local region.  Off by default; when off, no neighbor data is collected and repair behavior is unchanged. |
| `-neighbor_check_lambda` | Retained for option compatibility with the earlier gain/harm trade rule; the WNS-degradation rule ignores it. |
