...
| `-neighbor_check` | Enable the neighbor feasibility check: suppress a candidate move when the extra delay it imposes on an off-path neighbor (a fanin driver whose load grows, the other pin of a commutative-pin swap, a rebuffered fanout, or a load moved behind a split buffer) drives that neighbor's slack below zero by more than `lambda` times the move's own predicted gain.  Off by default; when off, no neighbor data is collected and repair behavior is unchanged. |
| `-neighbor_check_lambda` | Strictness of the neighbor check: a move is suppressed when the worst neighbor gives up more than `lambda` times the move's predicted gain in newly negative slack.  Smaller is more permissive (suppresses only egregious trades).  The default is 0.25. |
