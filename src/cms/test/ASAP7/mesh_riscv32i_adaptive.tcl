# mesh_riscv32i_adaptive.tcl
# 10x10 load-adaptive clock mesh on the riscv32i ORFS design.
# Companion test to mesh_aes_block_v4_adaptive.tcl; matches the clocksyn
# 10x10 grid run on the same RTL for apples-to-apples comparison
# (clocksyn/src/clocksyn_results/.../riscv32i_orfs/).

source "../helpers.tcl"

set orfs /home/wali2/OpenROAD-flow-scripts/flow
set results $orfs/results/asap7/riscv32i/base
...
# Same candidate library as the aes-block adaptive run.
...
# 10x10 grid (matches clocksyn run for apples-to-apples)
set grid_n 10
...
global_route -guide_file [make_result_file "riscv32i_mesh_adaptive.guide"] \
    -congestion_iterations 50
detailed_route -output_drc [make_result_file "riscv32i_mesh_adaptive_drc.rpt"] \
    -droute_end_iter 0
...
puts ""
puts "===== Adaptive buffer placement summary (riscv32i 10x10) ====="
array set counts {}
...
set spef_file [make_result_file "riscv32i_mesh_adaptive.spef"]
write_spef $spef_file

set script_dir [file dirname [file normalize [info script]]]

set spice_file [make_result_file "riscv32i_mesh_adaptive.spice"]
write_mesh_spice -clock $clk_name -output $spice_file -vdd 0.7 \
    -spice_models [list $script_dir/asap7_buf.spice]

set odb_file [make_result_file "riscv32i_mesh_adaptive.odb"]
write_db $odb_file
